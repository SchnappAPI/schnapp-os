...
- [ ] T2 Build wiring: `hooks/global-session-gate.sh` (any-repo ff-only pull of both repos,
  ...
  self-tests under `scripts/tests/`. Verify: new tests + full suite green, shellcheck clean.
- [ ] T3 Build shell: `shell/install.sh` (idempotent: renders `~/.claude/CLAUDE.md` from
