...
3. **A repeatable multi-step procedure the agent runs on request?** -> a **skill**
   (`skills/`, progressive-disclosure markdown). Single-purpose, one sentence to describe.
...
5. **A specialized read/build task better run in isolated context?** -> a **subagent**
   (the Agent/Task tool or a `agents/` type). Keeps big file reads out of the main window.
...
- **CLI beats MCP when a CLI exists.** An MCP server costs roughly an order of magnitude more tokens
  ...
  systems with no local CLI. Reinforces [context-budget](../../../skills/context-budget/SKILL.md)
...
- **Skill-ify repetition, but subtract too.** Anything done repeatedly becomes a skill *only if* it
  ...
  well-scoped components beat many broad ones - run the [context-budget](../../../skills/context-budget/SKILL.md)
