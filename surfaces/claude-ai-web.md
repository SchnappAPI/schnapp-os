...
- **Tools/credentials:** hosted MCP connectors only, enabled in Settings > Connectors. The
  **1Password** connector is the op-mcp portal `https://mcp.schnapp.bet/mcp` (Cloudflare Managed
  OAuth → Render); current op-mcp health is tracked in
  [`memory/credentials-state.md`](../memory/credentials-state.md) (canonical). Also GitHub + the
  Mac ops connector ("Schnapp Mac"). No local filesystem, shell, or hooks. To USE a secret, call
  the Mac's `op_run`/`op_inject` (value scrubbed); use op-mcp `op_read` only when the Mac is off
  AND the hosted connector is healthy (it returns the raw value into chat).
