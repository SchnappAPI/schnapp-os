 ...
 *
 * The connector serves private vault contents, so the endpoint MUST be protected.
 * The simplest portable gate is a static bearer token (a long random string)
 * checked in constant-ish time. For claude.ai registration you can either:
 *   - put this server behind a Cloudflare MCP portal / OAuth proxy (recommended), or
 ...
 * refuses to start (see index.ts) — we never run an open vault endpoint.
 */
