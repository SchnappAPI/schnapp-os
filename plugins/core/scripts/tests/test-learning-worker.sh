...
# real safety assertion: the dry-run must not dirty memory/ or plugins/core/rules/
git_before="$(git -C "$REPO_ROOT" status --porcelain -- memory/ plugins/core/rules/ 2>/dev/null)"
...
git_after="$(git -C "$REPO_ROOT" status --porcelain -- memory/ plugins/core/rules/ 2>/dev/null)"
