#!/usr/bin/env bash
# gen-catalog.sh — generate plugins/core/CATALOG.md, an inventory of the plugin's
# rules, skills, commands, and hooks (PLAN.md Part 9.2).
#
# The files under plugins/core/ are the single source of truth. This catalog is a
...
[ -d "$CORE" ] || { echo "FATAL: core not found: $CORE" >&2; exit 1; }
...

{
  ...
  echo
  echo "Inventory of \`plugins/core/\`, generated by [\`scripts/gen-catalog.sh\`](scripts/gen-catalog.sh)."
  ...
  echo
  for f in "$CORE"/rules/global/*.md; do
    [ -e "$f" ] || continue
  ...
  echo "## Agents"
  echo
  if ls "$CORE"/agents/*.md >/dev/null 2>&1; then
    for f in "$CORE"/agents/*.md; do echo "- **$(basename "${f%.md}")**"; done
  else
    echo "None yet."
  fi
} > "$TMP"
