...
# 9. nested-metadata memory file with updated bumped -> APPROVE (parser handles nested frontmatter)
reset
...
gate >/dev/null 2>&1; check "$?" 0 "nested-metadata memory bump -> APPROVE"

# 10. BYPASS: binary .md -> HOLD (cannot size-check)
reset
printf '\x00\x01\x02BINARYBLOB\x00' > "$tmp/memory/blob.md"
git -C "$tmp" add -A; git -C "$tmp" commit -qm "se binary"
out="$(gate 2>&1)"; check "$?" 1 "binary .md -> HOLD"
...
# 12. read-only: no working-tree changes left behind
