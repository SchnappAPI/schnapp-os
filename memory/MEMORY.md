...
- [Environment access](environment-access.md) — never-blocked playbook: blocked host = add to the env network allowlist (per-environment; canonical list in `docs/environment-and-access.md`); cloud `git push` is read-only (write via Mac or GitHub MCP); MCP flaps + `AskUserQuestion` unreliable here (retry / ask in text). ADR 0018.
