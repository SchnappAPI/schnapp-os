...
| `MEMORY_MCP_BEARER` | `/credential` | mcp | Static bearer gating the **memory-mcp** connector `/mcp` (cross-surface memory over the git-tracked lane) | Render `memory-mcp` env (`MEMORY_MCP_BEARER`); Claude Code header; for claude.ai/iPhone front via the Cloudflare portal like op-mcp. Created 2026-06-23; **deploy pending** |
| `MEMORY_GITHUB_PAT` `(to create)` | `/token` | github | Least-privilege GitHub token for memory-mcp: contents R/W on `SchnappAPI/schnapp-os` only (fine-grained PAT). Or reuse `GITHUB_PAT` (broader) | Render `memory-mcp` env (`GITHUB_TOKEN`) |
...
| 2026-06-23 | **Created `MEMORY_MCP_BEARER`** (`openssl rand -hex 32`, non-echoing) for the new memory-mcp connector | 1P item `MEMORY_MCP_BEARER/credential` (web-variables). **OWNER PENDING:** set in Render `memory-mcp` env + create `MEMORY_GITHUB_PAT` (or reuse `GITHUB_PAT`) + deploy + register connector (DEPLOY.md) | ~ |
