...
| `SCHNAPP_OS_PAT` | `/token` | github | Least-privilege GitHub token for memory-mcp: contents R/W on `SchnappAPI/schnapp-os` only (fine-grained PAT; owner created 2026-06-23) | Render `memory-mcp` env var **`GITHUB_TOKEN`** (the var name the code reads; the credential's identity is `SCHNAPP_OS_PAT`) |
...
| 2026-06-26 | **FLATTEN Phase B/C PENDING (owner go):** repoint live consumers in **`schnapp-bet`** then delete the drained bundles. To repoint: `.env.template` (Web App secrets→new items, config→`WEB_APP_CONFIG`, `Database/mssql_sa_password`→`MSSQL_SA_PASSWORD`); `claude.yml` (`Claude Code/oauth_token`→`CLAUDE_CODE_OAUTH_TOKEN`, `GitHub/pat_git_credentials`→`GITHUB_PAT/token`, `MCP Tokens/schnapp_mac`→`MAC_MCP_AUTH_TOKEN/credential`); `nba-backfill.yml` (`Web App/odds_api_key`→`ODDS_API_KEY`). **`Database/{server,database,username,password,trust_cert}` STAY (kept bundle) — ~18 ETL/grading workflows need NO change.** Then delete `Web App` (secret fields), `MCP Tokens`, `GitHub` bundles, `Claude Code`, `Anthropic`, and `Database/mssql_sa_password` field — each only after grep proves 0 refs. **ROTATION BACKLOG (separate): bundles never rotated post-leak (updated_at all 2026-05) → every copied value still compromised; rotate after flatten.** | (pending) | ☐ |
