...
| 2026-06-23 | **Rotating `OP_MCP_BEARER`** (leaked → fresh `openssl rand -hex 32`, non-echoing) — vault value stored; connector stays live on the OLD value until owner propagates all 3 legs in one window | 1P item `OP_MCP_BEARER/credential` (concealed). **OWNER (3 legs, same window)**: (1) Render `op-mcp` service env `OP_MCP_BEARER` = new value + redeploy; (2) Cloudflare MCP portal `mcp.schnapp.bet` → `op-mcp` server Custom header; (3) Code/Cowork client bearer for the off-Mac connector. Then verify `op_health`. Value source: `op://web-variables/OP_MCP_BEARER/credential` | ~ |
