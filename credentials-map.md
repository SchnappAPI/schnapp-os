...
| 2026-06-23 | **Created `MEMORY_MCP_BEARER`** (`openssl rand -hex 32`, non-echoing) for the new memory-mcp connector | 1P item `MEMORY_MCP_BEARER/credential` (web-variables). **OWNER PENDING:** set in Render `memory-mcp` env; store the owner-created fine-grained PAT `SCHNAPP_OS_PAT` in 1P (`SCHNAPP_OS_PAT/token`) and set it as Render env var `GITHUB_TOKEN`; deploy + register connector (DEPLOY.md) | ~ |
