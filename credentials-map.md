...
| 2026-06-26 | **ROTATION BACKLOG (still owed — flatten copied values, did NOT rotate):** every now-split secret is still the leaked 2026-05 value (`Web App`→6 items, `Database` incl. `MSSQL_SA_PASSWORD`, `ANTHROPIC_API_KEY`, `CLAUDE_CODE_OAUTH_TOKEN`), plus pre-existing `GITHUB_PAT`, Webshare, Cloudflare. Rotate per-secret next (some need provider portals). Also sanity-check `ADMIN_REFRESH_CODE` — copied as only 2 chars. | (pending rotation) | ☐ |
