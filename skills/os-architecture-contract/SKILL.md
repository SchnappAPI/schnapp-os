...
| Connector topology: Render pair Mac-independent, Mac duo tunneled, portal in front | Memory and secrets must survive the Mac being off (op-mcp + memory-mcp on Render); Mac powers (shell, SQL, services) are inherently Mac-hosted (mac-mcp :8765, obsidian-mcp :8767 behind cloudflared at `*.schnapp.bet`); the github leg is GitHub's official MCP server via portal-side headers, no Mac service (ADR 0036); claude.ai's connector UI is OAuth-only, so everything sits behind one Cloudflare OAuth portal (obsidian-mcp joined the static-bearer fleet 2026-07-18; portal slot live-verified same day) | [connectors/README.md](../../connectors/README.md), ADRs [0020](../../decisions/0020-portal-front-mac-github-mcp.md), [0036](../../decisions/0036-github-mcp-official-swap.md) |
