...
| **Off-Mac** (claude.ai, iPhone, Cowork) | The hosted **obsidian** MCP at `https://obsidian-mcp.schnapp.bet/mcp` (Mac-hosted FastMCP, static bearer `OBSIDIAN_MCP_AUTH_TOKEN` since 2026-07-18; reaches these surfaces via the Schnapp Portal once the owner adds its portal slot - pending. Full service detail in `schnapp-bet` → `docs/CONNECTIONS.md` → "Obsidian MCP"). Read with `search_notes`, `read_note`, `list_notes` (write tools `write_note`/`append_note`/`inbox_drop` also exist - read-only suffices for lookup). **Mac-dependency caveat:** this connector is hosted on the Mac, so if the Mac is off it is unavailable - fall back to the GitHub copy / this repo's `memory/` + `decisions/`. |
